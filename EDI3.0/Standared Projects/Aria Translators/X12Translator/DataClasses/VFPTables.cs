...
public class EDIPD
{
    ...
    public static int Update(DbConnection con,string UpdateCmd, string Wherecondition)
    {
        DbCommand UpdateCommand = con.CreateCommand();
        UpdateCommand.CommandText = "UPDATE EDIPD SET " + UpdateCmd;
        if (Wherecondition != null && Wherecondition.Trim() != "")
        {
            if (Wherecondition.Trim().ToLower().StartsWith("where"))
                UpdateCommand.CommandText += Wherecondition;
            else
                UpdateCommand.CommandText += " Where " + Wherecondition;
        }
        if (UpdateCommand.Connection.State == ConnectionState.Closed || UpdateCommand.Connection.State == ConnectionState.Broken)
            UpdateCommand.Connection.Open();
        var x = UpdateCommand.ExecuteNonQuery();
...
public class EDISV
{
	...
	public string ckey_val { get; set; }
	...
	public static EDISV Create(DataRow row)
	{
		EDISV newRow = new EDISV();
		foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
			if (property.CanWrite && row.Table.Columns.Contains(property.Name))
			{
	...
	public static List<EDISV> Create(DataTable Table)
	{
		List<EDISV> EDISVlist = new List<EDISV>();
		foreach (DataRow row in Table.Rows)
		{
			EDISV newObject = Create(row);
			EDISVlist.Add(newObject);
		}
		return EDISVlist;
	}
	...
	{
		string Fields = string.Empty, Values = string.Empty;
		foreach (PropertyInfo property in new EDISV().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into  [EDIMappingsHistory].[dbo].[edisv] (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	public int ExecuteVFPnsert(DbConnection con)
	{
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into EDISV (" + Fields + ") values(" + Values + ")";
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from EDISV ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<EDISV> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from EDISV ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCEDIXP
{
	...
	public string cversion { get; set; }
	public string loop_id { get; set; }
	public float f_order { get; set; }
	public string f_name { get; set; }
	public string f_value { get; set; }
	public string f_cond { get; set; }
	...
	public static SYCEDIXP Create(DataRow row)
	{
		SYCEDIXP newRow = new SYCEDIXP();
		foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
			if (property.CanWrite && row.Table.Columns.Contains(property.Name))
			{
				...
				{
					//property.SetValue(newRow, row[property.Name], null);
					property.SetValue(newRow, Convert.ChangeType(row[property.Name], property.PropertyType), null);
				}
				//FODA [END] phase2
			}
		}
	...
	public static List<SYCEDIXP> Create(DataTable Table)
	{
		List<SYCEDIXP> SYCEDIXP_Tlist = new List<SYCEDIXP>();
		foreach (DataRow row in Table.Rows)
		{
			SYCEDIXP newObject = Create(row);
			SYCEDIXP_Tlist.Add(newObject);
		}
		return SYCEDIXP_Tlist;
	}

	public int ExecuteSqlInsert(SqlConnection con)
	{
		string Fields = string.Empty, Values = string.Empty;
		foreach (PropertyInfo property in new SYCEDIXP().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into SYCEDIXP (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from SYCEDIXP ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<SYCEDIXP> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from SYCEDIXP ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCEDISV
{
	...
	public static SYCEDISV Create(DataRow row)
	{
		SYCEDISV newRow = new SYCEDISV();
        foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
            if (property.CanWrite && row.Table.Columns.Contains(property.Name) && row[property.Name] != DBNull.Value)
			{
				if (property.PropertyType == typeof(char) && row.Table.Columns[property.Name].DataType == typeof(string))
					property.SetValue(newRow, Convert.ToChar(row[property.Name]), null);
				else
	...
	public static List<SYCEDISV> Create(DataTable Table)
	{
		List<SYCEDISV> SYCEDISV_Tlist = new List<SYCEDISV>();
		foreach (DataRow row in Table.Rows)
		{
			SYCEDISV newObject = Create(row);
			SYCEDISV_Tlist.Add(newObject);
		}
		return SYCEDISV_Tlist;
	}

	public int ExecuteSqlInsert(SqlConnection con)
	{
		string Fields = string.Empty, Values = string.Empty;
		foreach (PropertyInfo property in new SYCEDISV().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into SYCEDISV (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from SYCEDISV ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<SYCEDISV> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from SYCEDISV ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCSEGHD
{
	...
	public static SYCSEGHD Create(DataRow row)
	{
		SYCSEGHD newRow = new SYCSEGHD();
		foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
			if (property.CanWrite && row.Table.Columns.Contains(property.Name))
			{
	...
	public static List<SYCSEGHD> Create(DataTable Table)
	{
		List<SYCSEGHD> SYCSEGHD_Tlist = new List<SYCSEGHD>();
		foreach (DataRow row in Table.Rows)
		{
			SYCSEGHD newObject = Create(row);
			SYCSEGHD_Tlist.Add(newObject);
		}
		return SYCSEGHD_Tlist;
	}

	public int ExecuteSqlInsert(SqlConnection con)
	{
		string Fields = string.Empty, Values = string.Empty;
		foreach (PropertyInfo property in new SYCSEGHD().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into SYCSEGHD (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from SYCSEGHD ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<SYCSEGHD> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from SYCSEGHD ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCSEGDT
{
    ...
    public char cusage { get; set; }
	public string desc1 { get; set; }
	public string cfld_name { get; set; }
	public int F_Order { get; set; }
	public string crltd_nam { get; set; }
    ...
    public string cdata_typ { get; set; }
	...
	public static SYCSEGDT Create(DataRow row)
	{
		SYCSEGDT newRow = new SYCSEGDT();
		foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
			if (property.CanWrite && row.Table.Columns.Contains(property.Name))
			{
	...
	public static List<SYCSEGDT> Create(DataTable Table)
	{
		List<SYCSEGDT> SYCSEGDT_Tlist = new List<SYCSEGDT>();
		foreach (DataRow row in Table.Rows)
		{
			SYCSEGDT newObject = Create(row);
			SYCSEGDT_Tlist.Add(newObject);
		}
		return SYCSEGDT_Tlist;
	}

	public int ExecuteSqlInsert(SqlConnection con)
	{
		string Fields = string.Empty, Values = string.Empty;
		foreach (PropertyInfo property in new SYCSEGDT().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into SYCSEGDT (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from SYCSEGDT ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<SYCSEGDT> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from SYCSEGDT ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCVICS
{
	...
	public string loop_id { get; set; }
	public string cfld_name { get; set; }
	public string ccode_no { get; set; }
	public string desc1 { get; set; }
	...
	public static SYCVICS Create(DataRow row)
	{
		SYCVICS newRow = new SYCVICS();
		foreach (PropertyInfo property in newRow.GetType().GetProperties())
		{
			if (property.CanWrite && row.Table.Columns.Contains(property.Name))
			{
	...
	public static List<SYCVICS> Create(DataTable Table)
	{
		List<SYCVICS> SYCVICS_Tlist = new List<SYCVICS>();
		foreach (DataRow row in Table.Rows)
		{
			SYCVICS newObject = Create(row);
			SYCVICS_Tlist.Add(newObject);
		}
		return SYCVICS_Tlist;
	}

	public int ExecuteSqlInsert(SqlConnection con)
	{
		string Fields = string.Empty, Values = string.Empty;
        foreach (PropertyInfo property in new SYCVICS().GetType().GetProperties())
		{
			bool valueUsed = false;
			object Value = property.GetValue(this, null);
			if (Value == null) continue;
		...
		Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
		Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
		string sql = "Insert into SYCVICS (" + Fields + ") values(" + Values + ")";
		SqlCommand cmd = con.CreateCommand();
		cmd.CommandText = sql;
		if (con.State == ConnectionState.Closed)
			con.Open();
	...
	{
		DbCommand deleteCommand = con.CreateCommand();
		deleteCommand.CommandText = "Delete from SYCVICS ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				deleteCommand.CommandText += Wherecondition;
	...
	public static List<SYCVICS> Select(DbConnection con, string Wherecondition)
	{
		DbCommand cmd = con.CreateCommand();
		cmd.CommandText = "Select * from SYCVICS ";
		if (Wherecondition != null && Wherecondition.Trim() != "")
		{
			if (Wherecondition.Trim().ToLower().StartsWith("where"))
				cmd.CommandText += Wherecondition;
...
public class SYCSEGVA
{
    #region Properties

    public string ceditrntyp { get; set; }
    public string loop_id { get; set; }
    public string cvar_name { get; set; }
    public string desc1 { get; set; }
    ...
    public static SYCSEGVA Create(DataRow row)
    {
        SYCSEGVA newRow = new SYCSEGVA();
        foreach (PropertyInfo property in newRow.GetType().GetProperties())
        {
            if (property.CanWrite && row.Table.Columns.Contains(property.Name))
            {
    ...
    public static List<SYCSEGVA> Create(DataTable Table)
    {
        List<SYCSEGVA> SYCSEGVA_Tlist = new List<SYCSEGVA>();
        foreach (DataRow row in Table.Rows)
        {
            SYCSEGVA newObject = Create(row);
            SYCSEGVA_Tlist.Add(newObject);
        }
        return SYCSEGVA_Tlist;
    }

    public int ExecuteSqlInsert(SqlConnection con)
    {
        string Fields = string.Empty, Values = string.Empty;
        foreach (PropertyInfo property in new SYCSEGVA().GetType().GetProperties())
        {
            bool valueUsed = false;
            object Value = property.GetValue(this, null);
            if (Value == null) continue;
        ...
        Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
        Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
        string sql = "Insert into SYCSEGVA (" + Fields + ") values(" + Values + ")";
        SqlCommand cmd = con.CreateCommand();
        cmd.CommandText = sql;
        if (con.State == ConnectionState.Closed)
            con.Open();
    ...
    {
        DbCommand deleteCommand = con.CreateCommand();
        deleteCommand.CommandText = "Delete from SYCSEGVA ";
        if (Wherecondition != null && Wherecondition.Trim() != "")
        {
            if (Wherecondition.Trim().ToLower().StartsWith("where"))
                deleteCommand.CommandText += Wherecondition;
    ...
    public static List<SYCSEGVA> Select(DbConnection con, string Wherecondition)
    {
        DbCommand cmd = con.CreateCommand();
        cmd.CommandText = "Select * from SYCSEGVA ";
        if (Wherecondition != null && Wherecondition.Trim() != "")
        {
            if (Wherecondition.Trim().ToLower().StartsWith("where"))
                cmd.CommandText += Wherecondition;
