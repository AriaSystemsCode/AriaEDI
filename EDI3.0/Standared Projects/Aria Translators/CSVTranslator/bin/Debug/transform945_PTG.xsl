    ...
    <Root>
      <xsl:for-each select="HEADER">
        ...
        <ShippingOrderHeader_T>
          <ShippingOrder><xsl:value-of select="$headerOrderId"/></ShippingOrder>
          ...
          <xsl:if test="USR_UOM != '' and USR_UOM">
            <WeightUOMCode><xsl:value-of select="USR_UOM"/></WeightUOMCode>
          </xsl:if>
        ...
        <xsl:if test="normalize-space(START_PICK_DATE) != ''">
          <ShippingOrderDates_T>
            <Date><xsl:value-of select="normalize-space(START_PICK_DATE)" /></Date>
            <Type>Start</Type>
            <ShippingOrder><xsl:value-of select="$headerOrderId"/></ShippingOrder>
          </ShippingOrderDates_T>
        </xsl:if>
        ...
        <xsl:if test="normalize-space(DATE_SHIPPED) != ''">
          <ShippingOrderDates_T>
            <Date><xsl:value-of select="normalize-space(DATE_SHIPPED)" /></Date>
            <Type>ship date</Type>
            <ShippingOrder><xsl:value-of select="$headerOrderId"/></ShippingOrder>
          </ShippingOrderDates_T>
        </xsl:if>
        ...
        <xsl:for-each select="/SHIPMENTUPLOAD/HEADER/DETAIL[normalize-space(ORDER_ID) = $headerOrderId]">
          <ShippingOrderItem_T>
            <ShippingOrder><xsl:value-of select="ORDER_ID"/></ShippingOrder>
            <SenderID><xsl:value-of select="WAREHOUSE_ID"/></SenderID>
            <QuantityOrdered><xsl:value-of select="PIECES_ORDERED"/></QuantityOrdered>
