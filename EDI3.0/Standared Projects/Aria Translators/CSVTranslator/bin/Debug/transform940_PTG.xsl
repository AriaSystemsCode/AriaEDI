    ...
    <xsl:template match="/NewDataSet">
        <ORDERDOWNLOAD>
            <HEADER>
				...
				<xsl:if test="ShippingOrderDates_T/Carrierservices 
              and normalize-space(ShippingOrderDates_T/Carrierservices) != '' 
              and ShippingOrderDates_T/Carrierservices/CCARRIERID 
              and normalize-space(ShippingOrderDates_T/Carrierservices/CCARRIERID) != ''
			  and ShippingOrderDates_T/Carrierservices/shipvia 
              and normalize-space(ShippingOrderDates_T/Carrierservices/shipvia) != ''"
			  >
                <SAN_SHIPMENT>
                </SAN_SHIPMENT>
                <FREIGHT_TERMS>
                    <xsl:choose>
						...
						<xsl:when test="ShippingOrderDates_T.ShipVia = 'UPSBLP'">P</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.ShipVia = 'UPSBLF'">C</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.ShipVia = 'UPSBLB'">T</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.ShipVia = 'UPSBLC'">R</xsl:when>
                ...
                <SHIP_METHOD>
                    <xsl:choose>
                        <xsl:when test="ShippingOrderDates_T.Carrierservices = 'UPS Next Day'">U01</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.Carrierservices = 'UPS 2ND DAY AIR'">U02</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.Carrierservices = 'UPS Ground US'">U03</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.Carrierservices = '01'">U01</xsl:when>
						<xsl:when test="ShippingOrderDates_T.Carrierservices = '02'">U02</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.Carrierservices = '03'">U03</xsl:when>
					   <xsl:otherwise>
				...
				<CARRIER_ID>
                    <xsl:choose>
                        <xsl:when test="ShippingOrderDates_T.CCARRIERID = 'UPS'">UPS</xsl:when>
                        <xsl:when test="ShippingOrderDates_T.CCARRIERID = 'FEDEX'">FDX</xsl:when>
					   <xsl:otherwise>
