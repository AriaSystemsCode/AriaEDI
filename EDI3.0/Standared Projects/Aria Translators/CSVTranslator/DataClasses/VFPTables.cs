...
public class EDILIBHD
{
    ...
    public string cedt_ver { get; set; }
    //Derby - Adding new MStatus field.[Start]
    public string mstatus { get; set; }

    //Derby - Adding new MStatus field.[End]
    #endregion
...
public class CTKTRCVH
{
    ...
    public string STOREPONO { get; set; }

    public string HANGER { get; set; }

    public string TICKET { get; set; }

    public string TMPRCVNUM { get; set; }
    ...
    public int NTOTCAN { get; set; }

    public int NTOTDAM { get; set; }

    public float NTOTSTK { get; set; }
    ...
    {
        string Fields = string.Empty, Values = string.Empty;
        foreach (PropertyInfo property in new CTKTRCVH().GetType().GetProperties())
        {
            bool valueUsed = false;
            object Value = property.GetValue(this, null);
            if (Value == null) continue;
        ...
        Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
        Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
        string sql = "Insert into CTKTRCVH (" + Fields + ") values(" + Values + ")";
        SqlCommand cmd = con.CreateCommand();
        cmd.CommandText = sql;
        if (con.State == ConnectionState.Closed)
            con.Open();
...
public class CTKTRCVL
{
    ...
    public float UNITSPERPACK { get; set; }

    public float TOTALNOOFCRTNS { get; set; }

    public string CCARTON { get; set; }

    public string CRETSTY { get; set; }

    public string CSTYGRADE { get; set; }
    ...
    public string Contnr { get; set; }

    public string PONO { get; set; }

    public float nLength { get; set; }

    public float nWidth { get; set; }

    public float nHeight { get; set; }

    public float nWeigth { get; set; }
    ...
    // Adding New Variable To Check Is Prepack Or Not [MSH][Start]
    public string CISPREPK { get; set; }
    // Adding New Variable To Check Is Prepack Or Not [MSH][End]
    ...
    {
        string Fields = string.Empty, Values = string.Empty;
        foreach (PropertyInfo property in new CTKTRCVL().GetType().GetProperties())
        {
            bool valueUsed = false;
            object Value = property.GetValue(this, null);
            if (Value == null) continue;
            ...
            // Add float Type To Be Generated In Insert Statement [MSH][Start]
            //else if (Value.GetType() == typeof(int) || Value.GetType() == typeof(decimal) || Value.GetType() == typeof(double) || Value.GetType() == typeof(Int64) || Value.GetType() == typeof(Int16))
            else if (Value.GetType() == typeof(float) || Value.GetType() == typeof(int) || Value.GetType() == typeof(decimal) || Value.GetType() == typeof(double) || Value.GetType() == typeof(Int64) || Value.GetType() == typeof(Int16))
            // Add float Type To Be Generated In Insert Statement [MSH][End]
            {
                Values += Value;
                valueUsed = true;
            }
        ...
        Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
        Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
        string sql = "Insert into CTKTRCVL (" + Fields + ") values(" + Values + ")";
        SqlCommand cmd = con.CreateCommand();
        cmd.CommandText = sql;
        if (con.State == ConnectionState.Closed)
            con.Open();
...
/**********************************/
// MSH - 945 
public class ShippingOrderHeader_T
{
    #region Properties
    ...
    public string PalletExchangeInstruction { get; set; }
    public string UnitLoadOptionCode { get; set; }
    public string UnitLoadOption { get; set; }
    ...
    public static ShippingOrderHeader_T Create(DataRow row)
    {
        ShippingOrderHeader_T newRow = new ShippingOrderHeader_T();
        foreach (PropertyInfo property in newRow.GetType().GetProperties())
        {
            if (property.CanWrite && row.Table.Columns.Contains(property.Name))
            {
    ...
    public static List<ShippingOrderHeader_T> Create(DataTable Table)
    {
        List<ShippingOrderHeader_T> ShippingOrderHeader_T_list = new List<ShippingOrderHeader_T>();
        foreach (DataRow row in Table.Rows)
        {
            ShippingOrderHeader_T newObject = Create(row);
            ShippingOrderHeader_T_list.Add(newObject);
        }
        return ShippingOrderHeader_T_list;
    }

    public int ExecuteSqlInsert(SqlConnection con)
    {
        string Fields = string.Empty, Values = string.Empty;
        foreach (PropertyInfo property in new ShippingOrderHeader_T().GetType().GetProperties())
        {
            bool valueUsed = false;
            object Value = property.GetValue(this, null);
            if (Value == null) continue;
        ...
        Values = Values.Length > 0 ? Values.Remove(Values.Length - 1) : Values;
        Fields = Fields.Length > 0 ? Fields.Remove(Fields.Length - 1) : Fields;
        string sql = "Insert into ShippingOrderHeader_T (" + Fields + ") values(" + Values + ")";
        SqlCommand cmd = con.CreateCommand();
        cmd.CommandText = sql;
        if (con.State == ConnectionState.Closed)
            con.Open();
    ...
    {
        DbCommand deleteCommand = con.CreateCommand();
        deleteCommand.CommandText = "Delete from ShippingOrderHeader_T ";
        if (Wherecondition != null && Wherecondition.Trim() != "")
        {
            if (Wherecondition.Trim().ToLower().StartsWith("where"))
                deleteCommand.CommandText += Wherecondition;
    ...
    public static List<ShippingOrderHeader_T> Select(DbConnection con, string Wherecondition)
    {
        DbCommand cmd = con.CreateCommand();
        cmd.CommandText = "Select * from ShippingOrderHeader_T ";
        if (Wherecondition != null && Wherecondition.Trim() != "")
        {
            if (Wherecondition.Trim().ToLower().StartsWith("where"))
                cmd.CommandText += Wherecondition;
...
public class SYCEDIXP
{
    ...
    public string cadd_user { get; set; }
    public string cadd_time { get; set; }
    public string dadd_date { get; set; }
    public string llok_stat { get; set; }
    public string clok_user { get; set; }
    public string dlok_date { get; set; }
    public string clok_time { get; set; }
    public string network { get; set; }
    public string VER { get; set; }
    public string cedit_user { get; set; }
    public string dedit_date { get; set; }
    public string cedit_time { get; set; }
    public string cadd_ver { get; set; }
    public string cedt_ver { get; set; }
