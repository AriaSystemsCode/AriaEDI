...
namespace CSVTranslator
{
    public class SENDTRANSACTIONS
    {
        ...
        {
            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", "in FillSegmentsData ");
          ...
          " ORDER BY [SEGMENT_ORDER]";
            //check if the mapping exists on the client level first
            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", Segmentswhere);
            _segmentsList = TRANSACTION_SEGMENTS_T.Select(AriaConnection.ClientMasterConnection, Segmentswhere);

            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", AriaConnection.ClientMasterConnection.ConnectionString  + " AriaConnection.ClientMasterConnection");

            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", _segmentsList.Count.ToString() + " client master");
            if (_segmentsList.Count == 0)
            {   // if not found on client level, then check on the edi mappings db (shared level).
                AriaConnection.ClientMasterConnection.Open();
                AriaConnection.ClientMasterConnection.ChangeDatabase("EDIMAPPINGS");
                _segmentsList = TRANSACTION_SEGMENTS_T.Select(AriaConnection.ClientMasterConnection, Segmentswhere);
                AriaConnection.ClientMasterConnection.Close();
                //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", _segmentsList.Count.ToString() + " EDI mappings");
            }
                if (_segmentsList.Count == 0)
            {
            ...
            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", _segmentsMappingList.Count.ToString() + " segments client master");
            ...
            //System.IO.File.AppendAllText(@"d:\shared\aria3edi\edi\outbox\log.txt", _segmentsMappingList.Count.ToString() + " segments edi mapping");
            if (_segmentsMappingList.Count == 0)
            {
        ...
        public void WriteLoop(string MapSet, string MapVersion, string LoopId, SqlCommand ConnectionString, StreamWriter FileStreem, int HeaderSegmentWritten)
        {
            ...
            foreach (TRANSACTION_SEGMENTS_T SegmentObj in currentLoopList)
            {
                ...
                foreach (var SegmentMap in Segments_Map_list)
                {
                    if (SegmentMap.FIELD_ORDER.Value == currentAfield)
                    {
                        ...
                        if (SegmentMap.VALUE.Contains('"'))
                            FieldsArray[FieldsArray.Count - 1] = SegmentMap.VALUE.Replace("\"", "");
                    }
                    else
                    {
                        ...
                        else if (SegmentMap.VALUE.Contains('"'))
                            FieldsArray.Add( SegmentMap.VALUE.Replace("\"", ""));
                        else
                            FieldsArray.Add(null);
                    }
        ...
        public void WriteRecord(StreamWriter FileStreem)
        {
            ...
            if (!string.IsNullOrEmpty(lcFileLine))
            { FileStreem.WriteLine(lcFileLine); }
        }
