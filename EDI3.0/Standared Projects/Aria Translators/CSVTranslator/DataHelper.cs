...
class DataHelper
{
    ...
    public static bool GetChildData(DataRow HeaderRow, List<string> ChildTablesNames, Dictionary<string, List<DataRow>> retData, bool successedBefore)
    {
        ...
        {
            foreach (DataRelation relation1 in HeaderRow.Table.ChildRelations)
            {
                List<DataRow> relationData1 = new List<DataRow>();

                relationData1.AddRange(HeaderRow.GetChildRows(relation1));
                if (successedBefore)
                {
                ...
                status = false;
                status = ChildTablesNames.Contains(relation1.ChildTable.TableName);
                if (!status)
                {
                    status = GetChildData(relationData1, ChildTablesNames, retData);
                }
                if (status)
                {
                    if (!retData.Keys.Contains(relation1.ChildTable.TableName))
                        retData.Add(relation1.ChildTable.TableName, new List<DataRow>());
                    foreach (DataRow relRow in relationData1)
                        if (!retData[relation1.ChildTable.TableName].Contains(relRow))
                            retData[relation1.ChildTable.TableName].Add(relRow);
                    if (!retData.Keys.Contains(HeaderRow.Table.TableName))
                        retData.Add(HeaderRow.Table.TableName, new List<DataRow>());
                    if (!retData[HeaderRow.Table.TableName].Contains(HeaderRow))
                        retData[HeaderRow.Table.TableName].Add(HeaderRow);
                    ChildTablesNames.Remove(relation1.ChildTable.TableName);
    ...
    public static void GetChildData1(DataRow HeaderRow, Dictionary<string, List<DataRow>> retData)
    {
        ...
        foreach (DataRelation relation in HeaderRow.Table.ChildRelations)
        {
             ...
             if (!retData.Keys.Contains(relation.RelationName))
                 retData.Add(relation.RelationName, new List<DataRow>());


             foreach (DataRow relRow in relationData)
                 if (!retData[relation.RelationName].Contains(relRow))
                     retData[relation.RelationName].Add(relRow);
    ...
    public static void RemoveChildFromCurrentLoop(DataRow HeaderRow, Dictionary<string, List<DataRow>> retData)
    {
        List<DataRow> relationData = new List<DataRow>();
        foreach (DataRelation relation in HeaderRow.Table.ChildRelations)
        {
            if (retData.Keys.Contains(relation.RelationName))
                retData.Remove(relation.RelationName);
        }
    }
