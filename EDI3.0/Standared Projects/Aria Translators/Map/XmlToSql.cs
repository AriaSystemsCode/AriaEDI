...
namespace Map
{
    ...
    public class XmlToSqlClass
    {
            ...
            try
            {
                ...
                if (needDataSetOnly == false)
                {
                    if (HasMapping)
                    {
                        ...
                        SetRelations();
                        CheckErrors();
                        //dataSetTarget.EnforceConstraints = false;
                        FillTableData();
                        ContinueCheck();
        ...
        private void FillData(DataTable targetDataTable)
        {
            ...
            // add dictionary to hold the relation parent for child table if mentioned in the mapping
            Dictionary<string, string> childParentrelation = new Dictionary<string, string>();
            ...
            foreach (XmlNode node in Mapnodes)
            {
                if (node[MapXML.SrcTable].InnerText.Trim() != "")
                {
                    ...
                    HasFixed = true;
                // add dictionary to hold the relation parent for child table if mentioned in the mapping
                if (node["SrcTableRelation"] !=null && node["SrcTableRelation"].InnerText.Trim() != "")
                {
                    childParentrelation.Add(node["SrcTable"].InnerText.Trim().ToUpper(), node["SrcTableRelation"].InnerText.Trim());
                }
            }
            ...
            foreach (string childtable in childtables)
            {
                ...
                foreach (string parenttable in parenttables)
                {
                    ...
                    RelationFound = isRelationExist((childParentrelation.ContainsKey(childtable.ToUpper())? childParentrelation[childtable]:""), childrelations, parenttable, ref ParentToChildRelations);
                    if (RelationFound)
                        break;
                }
            }
        ...
        /// <returns></returns>
        private bool isRelationExist(string childParentrelation, DataRelationCollection relations, string searchedTable, ref List<DataRelation> outRelations)
        {
            if (string.IsNullOrEmpty(childParentrelation) == false)
            {
                foreach (DataRelation relation in relations)
                {
                    if (relation.ParentTable.TableName.ToUpper() == childParentrelation.ToUpper())
                    {
