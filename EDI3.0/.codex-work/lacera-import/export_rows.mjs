...
const values = sheet.getUsedRange(true).values.slice(1);
...
const excelSerial = (text) => {
  ...
  const utc = Date.UTC(2000 + Number(m[3]), month[m[2]], Number(m[1]));
...
console.log(JSON.stringify({ rowCount: normalized.length, rows: normalized }));
