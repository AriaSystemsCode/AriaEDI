...
*E302357,1 TMI [Start] Copy below to avoide error "Illage redifinition of the variable gcBaseWind"
*RESTORE FROM (lcMemoFile) ADDI
*E302357,1 TMI [End  ] 

*PARAMETERs are sent from Manual Packing List program or Pack & Scan program ;
* tcEDIAct,tcEDIShp,tcEDICmp,tcEDIPrtNm,tcEDIBolNo,tcEDIPckNo,tnEDICrtNo,tcEDIVer,tlEDIDetLb,tcEDIDetVr
...
*E302357,1 TMI [Start] restore saved variables
...
IF TYPE('oAriaApplication') = "O"
   oAriaApplication.SysPath = lcCurPath + '\SYSFILES\'
   oAriaApplication.DataDir = lcCurPath + '\DBFS\'+  alltRIM(tcEDICmp) + '\'
   oARiaApplication.Init()
   oAriaApplication.ActiveCompanyID=alltRIM(tcEDICmp)
   oAriaApplication.SetEnvironment()
   oARiaApplication.SystemSetup()
   oAriaApplication.ChangeCompany(alltRIM(tcEDICmp))
   *EDI3
   *Do Form screens\as\ebvshplb.scx WITH  .T.,ALLTRIM(tcEDIAct),ALLTRIM(tcEDIShp) ,alltRIM(tcEDIPrtNm), ALLTRIM(tcEDIBolNo), ALLTRIM(tcEDIPckNo),tnEDICrtNo,ALLTRIM(tcEDIVer), tlEDIDetLb, IIF(tlEDIDetLb,ALLTRIM(tcEDIDetVr),'')
   lcAria4XPPath = lcCurPath
  ...
  *E302357,1 TMI [Start] Add the call to the "SendUccLabel" class
  =lfOpnFls()
  oUccLabels = CREATEOBJECT("SendUccLabels")
  tcEDIShp = oUccLabels.Do(tcEDIBOLNo,'A27')
  RELEASE oUccLabels
  =lfSetRel()  
  *E302357,1 TMI [End  ] 
...
IF UPPER(RIGHT(lcClassFileName, 3)) = "VCX"
  ...
  IF INLIST(UPPER(lcClassFileName), "MAIN.VCX", "GLOBALS.VCX", "UTILITY.VCX")
    lcCommand = 'IN '+ lcCurPath + '\PRGS\' + 'PRNLABEL.EXE'
  ENDIF
...
*:* Date        : 01/30/2007
*:* Purpose     : open the following files that are not opened when called from Aria27
*:***************************************************************************
*E302357,1 TMI 
FUNCTION lfOpnFls
...
*- Open system files
IF !USED('SYCTRIGG') 
  USE (oAriaApplication.SysPath+'\SYCTRIGG') IN 0 SHARE ORDER TAG OBJEVENT
ENDIF
IF !USED('SYCASNHD') 
  USE (oAriaApplication.SysPath+'\SYCASNHD') IN 0 SHARE ORDER TAG VERPRT
ENDIF
IF !USED('SYCASNDT') 
  USE (oAriaApplication.SysPath+'\SYCASNDT') IN 0 SHARE ORDER TAG 
ENDIF
IF !USED('CUSTDEPT') 
  USE (oAriaApplication.DataDir+'\CUSTDEPT') IN 0 SHARE ORDER TAG 
ENDIF

*- Open data files
IF !USED('EDITRANS') 
  USE (oAriaApplication.DataDir+'\EDITRANS') IN 0 SHARE ORDER TAG TYPEKEY
ENDIF
IF !USED('CODES') 
  USE (oAriaApplication.DataDir+'\CODES') IN 0 SHARE ORDER TAG 
ENDIF

*- always Open the temp files created in lfPrepPck IN ALPLIST.PRG since printing from aria27 can be called only in edit mode
USE (oAriaApplication.Workdir+lcTmpBolH) IN 0 SHARED ALIAS BOL_HDR  ORDER TAG BOL_HDR
USE (oAriaApplication.Workdir+lcTmpBolL) IN 0 SHARED ALIAS BOL_LIN  ORDER TAG BOL_LIN
USE (oAriaApplication.Workdir+lcTmPckH ) IN 0 SHARED ALIAS PACK_HDR ORDER TAG PACK_HDR
USE (oAriaApplication.Workdir+lcTmPckL ) IN 0 SHARED ALIAS PACK_LIN ORDER TAG PACK_LIN
...
*:* Developer   : TMI - TAREK MOHAMED IBRAHIM
*:* Date        : 01/31/2007
*:* Purpose     : set order and relation needed when using the class prnlabel
*:***************************************************************************
*E302357,1
FUNCTION lfSetRel
...
*- Set relations
SELECT BOL_HDR
SET RELATION TO bol_no INTO Bol_lin
SELECT BOL_LIN
SET RELATION TO pack_no INTO Pack_hdr
SELECT PACK_HDR
SET RELATION TO "O"+order INTO Ordhdr

SET ORDER TO TAG PACKSTYLE IN PACK_LIN
